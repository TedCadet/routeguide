...
pub fn load() -> Vec<crate::routeguide::Feature> {
    let dat_dir = std::path::PathBuf::from_iter([std::env!("CARGO_MANIFEST_DIR"), "data"]);
    let file = File::open(dat_dir.join("route_guide_db.json")).expect("failed to open data file");
